...
- **CRITICAL: Rate Limiting Fix for Shared Networks (Oct 8)**: Fixed critical production issue where IP-based rate limiting was blocking all users on shared networks (MIT WiFi, corporate networks, VPNs). The previous implementation tracked requests by IP address only, causing 60+ users on the same WiFi to share one rate limit bucket (100 requests/15min), leading to widespread 429 errors and even blocking admin access. Solution: Implemented hybrid user-based rate limiting where authenticated users (after login) are tracked by userId with higher limits (150 requests/15min per user), while anonymous users and pre-authentication requests share IP-based limits (100 requests/15min). Admin routes (/api/admin/*) are exempt from rate limiting entirely. This ensures each authenticated user gets their own rate limit bucket after login, preventing shared IP issues during assessment workflow while maintaining DDoS protection for anonymous traffic and login attempts. Configuration in `server/constants.ts` and `server/middleware/security.ts`.
- **PDF Generation Enhancements (Oct 8)**: Comprehensive improvements to executive brief PDF quality and reliability:
  - **Double Logo Fix**: Removed duplicate `addFooterLogoToAllPages()` calls, keeping only `finalizeFooters()` with centered logo rendering
