...
export interface OptionsStudioExportData {
  session: any;
  contextProfile?: any;
  enrichedOptionCards: any[];
  timestamp: string;
}

export async function generateOptionsStudioPDF(data: OptionsStudioExportData): Promise<void> {
  try {
    // Import jsPDF
    let jsPDF;
    try {
    ...
    doc.setFontSize(12);
    doc.setFont('helvetica', 'normal');
    doc.text('Options Studio Learning Summary', margin, 28);
    ...
    doc.setFont('helvetica', 'bold');
    doc.setTextColor(accentColor);
    doc.text('LEARNING JOURNEY SUMMARY', margin, currentY);
    currentY += 10;

    doc.setTextColor(primaryColor);

    // Use case and goals
    if (data.session.useCaseTitle) {
      doc.setFontSize(12);
      doc.setFont('helvetica', 'bold');
      ...
      doc.setFont('helvetica', 'normal');
      doc.setFontSize(10);
      const useCaseLines = doc.splitTextToSize(data.session.useCaseTitle, contentWidth - 20);
      doc.text(useCaseLines, margin + 5, currentY);
      currentY += useCaseLines.length * 4 + 5;
    }

    if (data.session.goals?.length > 0) {
      doc.setFontSize(12);
      doc.setFont('helvetica', 'bold');
      doc.text('Goals:', margin, currentY);
      currentY += 6;
      
      doc.setFont('helvetica', 'normal');
      doc.setFontSize(10);
      doc.text(data.session.goals.join(', '), margin + 5, currentY);
      currentY += 10;
    }
    ...
    doc.setFontSize(12);
    doc.setFont('helvetica', 'bold');
    doc.text(`Options Explored (${data.session.optionsViewed.length} of ${data.enrichedOptionCards.length}):`, margin, currentY);
    currentY += 6;
    
    doc.setFont('helvetica', 'normal');
    doc.setFontSize(10);
    const viewedOptions = data.enrichedOptionCards.filter(card => 
      data.session.optionsViewed.includes(card.id)
    );
    
    viewedOptions.forEach(option => {
      checkPageOverflow(8);
      doc.text(`• ${option.title}`, margin + 5, currentY);
      currentY += 4;
    });
    currentY += 5;
    ...
    checkPageOverflow(20);
    const correctCount = Object.values(data.session.misconceptionsAnswered || {}).filter(Boolean).length;
    doc.setFontSize(12);
    doc.setFont('helvetica', 'bold');
    doc.text(`Knowledge Check Results (${correctCount} correct):`, margin, currentY);
    currentY += 6;
    
    doc.setFont('helvetica', 'normal');
    doc.setFontSize(10);
    doc.text('Key myths corrected during your learning session', margin + 5, currentY);
    currentY += 10;

    // Reflections
    if (data.session.reflections?.promisingOptions) {
      checkPageOverflow(30);
      doc.setFontSize(12);
      doc.setFont('helvetica', 'bold');
      doc.text('Promising Options & Reasoning:', margin, currentY);
      currentY += 6;
      
      doc.setFont('helvetica', 'normal');
      doc.setFontSize(10);
      const reflectionLines = doc.splitTextToSize(data.session.reflections.promisingOptions, contentWidth - 10);
      doc.text(reflectionLines, margin + 5, currentY);
      currentY += reflectionLines.length * 4 + 8;
    }

    if (data.session.reflections?.priorityLenses?.[0]) {
      checkPageOverflow(15);
      doc.setFontSize(12);
      doc.setFont('helvetica', 'bold');
      doc.text('Most Important Lens:', margin, currentY);
      currentY += 6;
      ...
      const priorityLens = lensLabels[data.session.reflections.priorityLenses[0] as keyof typeof lensLabels] || data.session.reflections.priorityLenses[0];
      doc.text(priorityLens, margin + 5, currentY);
      currentY += 10;
    }
    ...
    doc.setFont('helvetica', 'bold');
    doc.setTextColor(accentColor);
    doc.text('SUGGESTED NEXT STEPS', margin + 5, currentY + 5);
    ...
    const nextSteps = [
      '• Review the detailed option cards for your two most promising patterns',
      '• Identify a small pilot project to test your chosen approach',
      '• Set up evaluation criteria based on your priority lens',
      '• Schedule follow-up discussion with your team about findings'
    ];
    
    let stepY = currentY + 12;
    nextSteps.forEach(step => {
      const lines = doc.splitTextToSize(step, contentWidth - 10);
      doc.text(lines, margin + 5, stepY);
    ...
    const link = document.createElement('a');
    link.href = url;
    link.download = `options-studio-summary-${Date.now()}.pdf`;
    link.style.display = 'none';
    document.body.appendChild(link);
