...
- **Auto-Complete Assessment on Results Page (Oct 6)**: Fixed infinite loop bug when results page loads with all 6 domains scored but assessment not marked complete. Solution: Implemented single-attempt auto-completion using ref guard that fires once per page load. On success, optimistically updates cache with `setQueryData` and invalidates as backup. On error, shows toast notification and keeps guard set (user must refresh to retry). This prevents infinite retry loops while ensuring assessments are properly completed.
